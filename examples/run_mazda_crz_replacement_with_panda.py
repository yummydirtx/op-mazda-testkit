...
for candidate in (REPO_ROOT, Path("/data/opendbc_test")):
  if candidate.exists():
...
try:
  ...
  helper_path = next(
    (
      candidate for candidate in (
        REPO_ROOT / "opendbc/car/mazda/longitudinal_experimental.py",
        Path("/data/opendbc_test/opendbc/car/mazda/longitudinal_experimental.py"),
      )
      if candidate.exists()
    ),
    None,
  ...
  patch_signal = module.patch_signal
...
STOCK_STOPGO_EVENTS_HOLD_ENTRY_SEQ: tuple[bytes, ...] = (
  bytes.fromhex("32acc16526cbabca"),
  bytes.fromhex("32acc16526cb5c19"),
  bytes.fromhex("32acc16526cb0d68"),
  bytes.fromhex("32acc16526cabeb8"),
  bytes.fromhex("32acc16526ca6f07"),
  bytes.fromhex("32acc16426ca1066"),
  bytes.fromhex("32acc16426c9b1c6"),
  bytes.fromhex("32acc16426c96215"),
  bytes.fromhex("32acc16426c91364"),
  bytes.fromhex("32acc16426c8c4b4"),
)
STOCK_STOPGO_EVENTS_HOLD_LATCH_SEQ: tuple[bytes, ...] = (
  bytes.fromhex("32ad015f26c00a35"),
  bytes.fromhex("32ad015f26c48bb0"),
  bytes.fromhex("32ad015f26cd7cb6"),
  bytes.fromhex("32ad017f26d66dbc"),
  bytes.fromhex("32ad017f26df6eb2"),
  bytes.fromhex("32ad017f26e85fb8"),
  bytes.fromhex("32ad017f26f140ce"),
  bytes.fromhex("32ad017f26fa41c4"),
  bytes.fromhex("32ad017f26fff20e"),
  bytes.fromhex("32ad017f26fff30d"),
)
STOCK_STOPGO_EVENTS_RELEASE_SEQ: tuple[bytes, ...] = (
  bytes.fromhex("32acc77f26fff348"),
  bytes.fromhex("32acc77f26fff447"),
  bytes.fromhex("32acc17f26fff54c"),
  bytes.fromhex("32acc17f26fff64b"),
  bytes.fromhex("32acc17f26fff74a"),
  bytes.fromhex("32acc17f26fff849"),
  bytes.fromhex("32acc17f26fff948"),
  bytes.fromhex("32acc17f26fffa47"),
)
...
def parse_args() -> argparse.Namespace:
  ...
  parser.add_argument("--inject-stock-hold-21f-burst", action="store_true",
                      help="When approaching a stop, replay a real stock 0x21f hold-entry + hold-latch burst captured from radar ACC.")
  parser.add_argument("--inject-stock-release-21f-burst", action="store_true",
                      help="After a stock 0x21f hold burst, replay a real stock 0x21f release burst on RES or gas.")
  parser.add_argument("--unsafe-force-gas-maybe-stopgo", action="store_true",
                      help="Unsafely patch only CRZ_EVENTS.GAS_MAYBE during stop/go transitions. CRZ_EVENTS checksum semantics remain unresolved.")
  parser.add_argument("--hold-21f-trigger-kph", type=float, default=1.0,
...
def apply_stock_stopgo_21f(command_set,
                           parser: CANParser | None,
                           args: argparse.Namespace,
                           mode: str,
                           seq_index: int) -> tuple[object, str, int]:
  if parser is None:
  ...
  if args.unsafe_force_gas_maybe_stopgo:
    ...
    if standstill and speed_kph <= args.hold_21f_latch_kph:
      ...
      gasmaybe_value = 1
    if gasmaybe_value is not None:
      raw_21f = patch_signal("CRZ_EVENTS", command_set.raw_21f, "GAS_MAYBE", gasmaybe_value)
      return replace(command_set, raw_21f=raw_21f), mode, seq_index

  if not args.inject_stock_hold_21f_burst and not args.inject_stock_release_21f_burst:
    return command_set, mode, seq_index

  if mode == "done" and speed_kph > args.hold_21f_rearm_kph:
    mode = "idle"
    seq_index = 0
  ...
  if mode == "hold_entry":
    raw_21f = STOCK_STOPGO_EVENTS_HOLD_ENTRY_SEQ[seq_index]
    seq_index += 1
    if seq_index >= len(STOCK_STOPGO_EVENTS_HOLD_ENTRY_SEQ):
      mode = "hold_latched"
      seq_index = 0
    return replace(command_set, raw_21f=raw_21f), mode, seq_index

  if mode == "hold_latched":
    raw_21f = STOCK_STOPGO_EVENTS_HOLD_LATCH_SEQ[seq_index]
    seq_index += 1
    if seq_index >= len(STOCK_STOPGO_EVENTS_HOLD_LATCH_SEQ):
      mode = "done"
      seq_index = 0
    return replace(command_set, raw_21f=raw_21f), mode, seq_index

  if mode == "release":
    raw_21f = STOCK_STOPGO_EVENTS_RELEASE_SEQ[seq_index]
    seq_index += 1
    if seq_index >= len(STOCK_STOPGO_EVENTS_RELEASE_SEQ):
      mode = "idle"
      seq_index = 0
    return replace(command_set, raw_21f=raw_21f), mode, seq_index
...
def print_status(prefix: str, parser: CANParser, rx_counts: Counter[int]) -> None:
  ...
  rx_gasmaybe = int(parser.vl["CRZ_EVENTS"]["GAS_MAYBE"])
  speed_kph = parser.vl["ENGINE_DATA"]["SPEED"]
  ...
  print(
    ...
    f"crz_speed={crz_speed_kph:.2f}kph standstill={standstill} brake_on={brake_on} gas={gas:.1f}% gasmaybe={rx_gasmaybe} "
...
def print_tx_status(prefix: str, command_set) -> None:
  ...
  tx_events_gasmaybe = decode_signal("CRZ_EVENTS", command_set.raw_21f, "GAS_MAYBE")
  ...
  print(
    ...
    f"tx_events_accel_cmd={tx_events_accel_cmd:.0f} tx_events_accel_low_res={tx_events_accel_low_res:.0f} tx_events_gasmaybe={tx_events_gasmaybe:.0f} "
